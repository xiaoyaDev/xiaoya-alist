...
def poll_qrcode_status(_token, stop, log_print):
    ...
    while not stop.is_set():
        ...
        _re = requests.get(
            f"{api_url}?client_id=532&v=1.2&token={_token}&request_id={uuid.uuid4()}",
            timeout=10,
        )
...
if __name__ == "__main__":
    ...
    logging.info("二维码生成中...")
    re = requests.get(
        f"https://uop.quark.cn/cas/ajax/getTokenForQrcodeLogin?client_id=532&v=1.2&request_id={uuid.uuid4()}",
        timeout=10,
    )  # noqa: E501
    if re.status_code == 200:
        ...
        qr.add_data(
            f"https://su.quark.cn/4_eMHBJ?token={token}&client_id=532&ssb=weblogin&uc_param_str=&uc_biz_str=S%3Acustom%7COPT%3ASAREA%400%7COPT%3AIMMERSIVE%401%7COPT%3ABACK_BTN_STYLE%400"
        )
        qr.make(fit=True)
